...
durante uma inalação, e depois durante uma exalação. Não tentem fazer,
...
duração de tempo de uma inalação e de uma expiração.
...
controlar. Estamos assim a usar a duração de tempo de uma inalação e de
uma exalação de modo a limitar o esforço apenas este período de tempo,
durante o qual mantemos a nossa atenção. Empenhem"-se em colocar um
esforço no princípio da inalação e em mantê"-lo. Depois façam o
mesmo durante a exalação e de novo na inalação. Eventualmente tudo se
...
afundarem em passividade. Isto requer esforço contínuo: uma inalação de
`\emph{Bud}' completamente clara na vossa mente, o próprio pensamento
presente e claro desde o princípio até ao fim da inalação. E
`\emph{dho}' na exalação. Nessa altura larguem tudo o resto. Esta agora
...
que deixa a mente dormente; energizem a mente de forma a que a inalação
em `\emph{Bud}' seja uma inalação lúcida, não apenas um som `\emph{Bud}'
...
sílaba completamente presente durante o período de uma inalação, do
princípio ao fim. Durante a exalação `\emph{dho}' pode ser realizado da
...
inalação - exalação, durante quinze minutos.
...
Estamos contentes com uma inalação, uma exalação\ldots{} temos todo o
tempo do mundo, só para ficar com uma inalação, desde o princípio ao
fim.
...
Será que podem estar somente com uma inalação? Estar satisfeitos com uma
simples exalação? Estar satisfeitos apenas com o breve espaço de tempo
...
simplesmente esse -- estar consciente durante uma inalação, durante o
período de tempo de uma inalação. Estabeleçam a vossa atenção no
...
necessidade para este tipo de coisas; na realidade, o Buddha disse
\ldots{} o Buddha nunca iria permitir isto, o Buddha\ldots{}; Eu sei
...
\ldots{}' O que quer que seja. Já não estamos preocupados com a
...
irrequietos, sentem"-se presos, enjaula- dos. Só pensam em saltar, fugir
...
agitação. Os robes que os \emph{bhikkhus} vestem não se ajustam ao subir
...
atenção. Ele gosta que se concentrem nele de uma maneira pací- fica e
...
corpo não inte- ressa, a mente é que conta.' Essa é uma atitude bastante
